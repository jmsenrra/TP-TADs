...
\tadOperacion{convertidoEnE}{posición /p, conj(posición) /estudiantes, conj(posición) /obstáculos}{bool}{}
\tadOperacion{captores}{posición /p, conj(posición) /agentes, conj(posición) obstáculos}{conj(posición)}{}
...
\tadOperacion{convertirEnEstudiantesR\footnotemark[5]}{posición /p, conj(posición) /estudiantes, conj(posición) /hippies, conj(posición) /obstáculos}{tupla(conj(posición), conj(posición))}{}
...
\tadOperacion{eNuevo\footnotemark[6]}{campus /c, posición p}{ tupla(conj(posición),conj(posición),dicc(posición,agente))}{$(\pi_2(p) = 1 \vee \pi_2(p) = alto(mapa(c))) \wedge 1 \leq \pi_1(p) \wedge \pi_1(p) \leq ancho(mapa(c)) \wedge p \notin obstaculos(mapa(c)) \wedge p \notin estudiantes(c) \wedge p \notin hippies(c) \wedge p \notin claves(agentes(c))$}
\tadOperacion{hNuevo\footnotemark[6]}{campus /c, posición p}{ tupla(conj(posición),conj(posición),dicc(posición,agente))}{$(\pi_2(p) = 1 \vee \pi_2(p) = alto(mapa(c))) \wedge 1 \leq \pi_1(p) \wedge \pi_1(p) \leq ancho(mapa(c)) \wedge p \notin obstaculos(mapa(c)) \wedge p \notin estudiantes(c) \wedge p \notin hippies(c) \wedge p \notin claves(agentes(c))$}
...
\tadAxioma{sancionarCaptores(das,cs)}{\IF vacío?(cs) THEN
  	as
ELSE
...
FI}
\tadAxioma{sumarCapturas(das,cs)}{\IF vacío?(cs) THEN
	as
ELSE
...
FI}) FI}
\tadAxioma{posSiguientes(p,ps)}{\IF vacío?(ps) THEN
	$\emptyset$
...
FI}) FI}
\tadAxioma{siguiente(p,ps)}{\IF vacío?(posSiguientes(p,ps)) THEN
	$\langle 0, 0 \rangle$
ELSE
